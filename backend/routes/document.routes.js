...
// Import middleware auth - giả sử bạn đã có
let authMiddleware;
try {
    authMiddleware = require('../middleware/authMiddleware');
} catch (error) {
    // Fallback nếu middleware ở vị trí khác
    try {
        authMiddleware = require('../middlewares/authMiddleware');
    } catch (error2) {
        console.warn('⚠️ Không tìm thấy authMiddleware, vui lòng kiểm tra đường dẫn');
        // Tạo middleware dummy cho testing
        authMiddleware = (req, res, next) => {
            // CẢNH BÁO: Đây chỉ là cho mục đích testing, không dùng trong production
            req.user = { id: '507f1f77bcf86cd799439011' }; // ObjectId giả
            next();
        };
    }
}
