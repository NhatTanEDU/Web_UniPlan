...
export const teamApi = {
  getTeams: async (params?: { /*...*/ }): Promise<{ teams: Team[]; total: number; pagination?: any }> => {
    const defaultParams = { sortBy: 'team_name', sortOrder: 'asc' as const, ...params };
    // API GET /teams của team.controller.js trả về { teams: teamsWithMemberCountAndRole, pagination }
    const backendResponse = await apiClient.get('/teams', { params: defaultParams }) as { teams: any[], pagination?: any, total?: number };

    const frontendTeams = backendResponse.teams.map((teamFromBackend: any) => {
      return {
        _id: teamFromBackend._id,
        name: teamFromBackend.team_name, // Map từ team_name
        description: teamFromBackend.description,
        type: mapBackendTypeToFrontendType(teamFromBackend.type), // Map type
        createdBy: teamFromBackend.created_by, // Giả sử backend đã populate created_by
        userRole: teamFromBackend.userRole,   // Controller getTeams đã thêm trường này
        createdAt: ensureDateObject(teamFromBackend.createdAt || teamFromBackend.created_at), // Ưu tiên createdAt, rồi đến created_at
        updatedAt: ensureDateObject(teamFromBackend.updatedAt || teamFromBackend.updated_at), // Ưu tiên updatedAt
        // Các trường tính toán từ getTeams như memberCount không thuộc interface Team này,
        // chúng sẽ được xử lý riêng ở component nếu cần, hoặc kiểu Team có thể mở rộng hơn.
        // isPublic: mapBackendTypeToFrontendType(teamFromBackend.type) === 'Public', // có thể thêm nếu cần
      } as Team; // Ép kiểu về Team (đảm bảo Team interface có đủ các trường trên)
    });
    return { teams: frontendTeams, total: backendResponse.pagination?.total || backendResponse.total || frontendTeams.length, pagination: backendResponse.pagination };
  },
