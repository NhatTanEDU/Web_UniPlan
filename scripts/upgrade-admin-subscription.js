...
// Load environment từ backend folder
require('dotenv').config({ path: path.join(__dirname, '../backend/.env') });

// Import models từ backend
const User = require('../backend/models/user.model');
const SubscriptionHistory = require('../backend/models/subscriptionHistory.model');
...
const upgradeUserSubscription = async (email, planType = 'monthly') => {
    try {
        ...
        const historyData = {
            user_id: user._id,
            action_type: 'upgrade',
            old_plan_type: oldPlan.plan_type,
...
const main = async () => {
    try {
        ...
        if (process.argv.includes('--confirm')) {
            ...
            console.log(`   node scripts/upgrade-admin-subscription.js ${email} ${planType} --confirm`);
